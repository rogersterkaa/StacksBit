...
(define-public (confirm-delivery
  ...
  (let (
    ...
    (let ((result (try! (as-contract (contract-call? .stacksbit-escrow release-funds payment-id token)))))
...
(define-public (raise-dispute (payment-id uint))
  (let (
    ...
    (asserts! (is-eq (some caller) (get customer payment)) ERR-NOT-CUSTOMER)
    (asserts! (is-eq (get status payment) "locked") ERR-WRONG-STATUS)
    (try! (as-contract (contract-call? .stacksbit-escrow flag-dispute payment-id)))
...
(define-public (resolve-dispute-refund (payment-id uint) (token <sip-010-trait>))
  (begin
    ...
    (try! (as-contract (contract-call? .stacksbit-escrow refund-customer payment-id token)))
    (ok true)
  )
)
