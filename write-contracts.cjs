...
(define-public (register-merchant
  ...
  (let ((caller tx-sender))
    ...
    (let ((merchant-id (try! (as-contract (contract-call? .stacksbit-merchants register-merchant caller business-name email)))))
...
(define-public (pay-invoice
  ...
  (let (
    ...
    (try! (as-contract (contract-call? .stacksbit-merchants lock-payment payment-id caller)))
...
(define-public (confirm-delivery
  ...
  (let (
    ...
    (let ((result (try! (contract-call? .stacksbit-escrow release-funds payment-id token))))
      (try! (as-contract (contract-call? .stacksbit-merchants settle-payment payment-id)))
      (print { event: "delivery-confirmed", payment-id: payment-id, customer: caller })
      (ok result)
    )
  )
)
...
(define-public (raise-dispute (payment-id uint))
  (let (
    ...
    (try! (as-contract (contract-call? .stacksbit-merchants dispute-payment payment-id)))
...
(define-public (withdraw (amount uint) (token <sip-010-trait>))
  (let (
    ...
    (asserts! (not (var-get contract-paused)) ERR-CONTRACT-PAUSED)
    (asserts! (> amount u0) ERR-INVALID-AMOUNT)
    (try! (as-contract (contract-call? .stacksbit-merchants deduct-balance merchant-id (contract-of token) amount)))
    (try! (as-contract (contract-call? token transfer amount tx-sender caller none)))
    (print { event: "merchant-withdrawal", merchant-id: merchant-id, merchant: caller, amount: amount })
    (ok true)
  )
)
...
console.log('gateway written:', fs.statSync('contracts/stacksbit-gateway.clar').size, 'bytes');